    ...
    while running:
        ...
        if int(model.t) != int(timeOfLastIntervention):
            ...
            if backlog_skipped_intervals:
                cadenceDayNumbers = [
                    int(i % cadence_cycle_length)
                    for i in numpy.arange(
                        start=timeOfLastIntervention, stop=int(model.t), step=1.0
                    )[1:]
                ] + cadenceDayNumbers
            ...
            for cadenceDayNumber in cadenceDayNumbers:
                ...
                currentPctInfected = (
                    model.total_num_infected()[model.tidx] / model.numNodes
                )

                if (
                    currentPctInfected >= intervention_start_pct_infected
                    and not interventionOn
                ):
                    interventionOn = True
                ...
                if interventionOn:
                    ...
                    if any(isolation_compliance_symptomatic_individual):
                        symptomaticNodes = numpy.argwhere(
                            (nodeStates == model.I_sym)
                        ).flatten()
                        for symptomaticNode in symptomaticNodes:
                            if isolation_compliance_symptomatic_individual[
                                symptomaticNode
                            ]:
                                ...
                                if isolation_groups is not None and any(
                                    ...
                                    isolationGroupmates = next(
                                        (
                                            ...
                                            if symptomaticNode in group
                                        ),
                                        None,
                                    )
                                    for isolationGroupmate in isolationGroupmates:
                                        if isolationGroupmate != symptomaticNode:
                                            if isolation_compliance_symptomatic_groupmate[
                                                isolationGroupmate
                                            ]:
                                                numSelfIsolated_symptomaticGroupmate += (
                                                    1
                                                )
                                                newIsolationGroup_symptomatic.append(
                                                    isolationGroupmate
                            ...
                            if isolation_groups is not None and any(
                                ...
                                isolationGroupmates = next(
                                    (
                                        group
                                        for group in isolation_groups
                                        if contactNode in group
                                    ),
                                    None,
                                )
                                for isolationGroupmate in isolationGroupmates:
                                    # if (isolationGroupmate != contactNode):
                                    if isolation_compliance_positive_contactgroupmate[
                                        isolationGroupmate
                                    ]:
                    ...
                    if any(testing_compliance_symptomatic):
                        ...
                        symptomaticPool = numpy.argwhere(
                            (testing_compliance_symptomatic == True)
                        ...
                        if len(symptomaticPool) > 0:
                            symptomaticSelection = symptomaticPool[
                                numpy.random.choice(
                                    len(symptomaticPool),
                                    min(numSymptomaticTests, len(symptomaticPool)),
                                    replace=False,
                                )
                            ]
                    ...
                    if cadenceDayNumber in testingDays:
                        ...
                        if any(testing_compliance_traced):
                            ...
                            numTracingTests = min(
                                len(tracingPool),
                                min(
                                    tests_per_day - len(symptomaticSelection),
                            ...
                            for trace in range(numTracingTests):
                                ...
                                if (
                                    (nodePositiveStatuses[traceNode] == False)
                        ...
                        if any(testing_compliance_random):
                            ...
                            testingPool = numpy.argwhere(
                                (testing_compliance_random == True)
                                & (nodePositiveStatuses == False)
                                & (nodeStates != model.R)
                            ...
                            testingPool_degreeWeights = numpy.power(
                                testingPool_degrees, random_testing_degree_bias
                            ) / numpy.sum(
                                numpy.power(
                                    testingPool_degrees, random_testing_degree_bias
                                )
                    ...
                    selectedToTest = numpy.concatenate(
                        (symptomaticSelection, tracingSelection, randomSelection)
                    ).astype(int)
                    ...
                    for i, testNode in enumerate(selectedToTest):
                        ...
                        if (
                            model.X[testNode] == model.S
                            or model.X[testNode] == model.Q_S
                        ):
                            pass
                        # Also assume that latent infections are not picked up by tests:
                        elif (
                            model.X[testNode] == model.E
                            or model.X[testNode] == model.Q_E
                        ):
                            pass
                        elif (
                            model.X[testNode] == model.I_pre
                            or model.X[testNode] == model.Q_pre
                            or model.X[testNode] == model.I_sym
                            or model.X[testNode] == model.Q_sym
                            or model.X[testNode] == model.I_asym
                            or model.X[testNode] == model.Q_asym
                        ):

                            if test_falseneg_rate == "temporal":
                                testNodeState = model.X[testNode][0]
                                testNodeTimeInState = model.timer_state[testNode][0]
                                if testNodeState in list(
                                    temporal_falseneg_rates.keys()
                                ):
                                    falseneg_prob = temporal_falseneg_rates[
                                        testNodeState
                                    ][
                                        int(
                                            min(
                            ...
                            if numpy.random.rand() < (1 - falseneg_prob):
                                ...
                                elif i < len(symptomaticSelection) + len(
                                    tracingSelection
                                ):
                                ...
                                if isolation_groups is not None and any(
                                    ...
                                    isolationGroupmates = next(
                                        (
                                            group
                                            for group in isolation_groups
                                            if testNode in group
                                        ),
                                        None,
                                    )
                                    for isolationGroupmate in isolationGroupmates:
                                        if isolationGroupmate != testNode:
                                            if isolation_compliance_positive_groupmate[
                                                isolationGroupmate
                                            ]:
                                ...
                                # ----------------------------------------
                                if (
                                    any(tracing_compliance)
                                    or any(isolation_compliance_positive_contact)
                                    or any(
                                        isolation_compliance_positive_contactgroupmate
                                    )
                                ):
                    ...
                    print(
                        ...
                        + str(numPositive_symptomatic)
                        + " positive (%.2f %%) +]"
                        % (
                            numPositive_symptomatic / numTested_symptomatic * 100
                            if numTested_symptomatic > 0
                            else 0
                        )
                    )
                    print(
                        "\t"
                        + str(numTested_tracing)
                        + "\ttested as traces        [+ "
                        + str(numPositive_tracing)
                        + " positive (%.2f %%) +]"
                        % (
                            numPositive_tracing / numTested_tracing * 100
                            if numTested_tracing > 0
                            else 0
                        )
                    )
                    print(
                        "\t"
                        + str(numTested_random)
                        + "\ttested randomly         [+ "
                        + str(numPositive_random)
                        + " positive (%.2f %%) +]"
                        % (
                            numPositive_random / numTested_random * 100
                            if numTested_random > 0
                            else 0
                        )
                    )
                    print(
                    ...
                    print(
                        "\t"
                        + str(numSelfIsolated_symptoms)
                        + " will isolate due to symptoms         ("
                        + str(numSelfIsolated_symptomaticGroupmate)
                        + " as groupmates of symptomatic)"
                    )
                    print(
                        "\t"
                        + str(numPositive)
                        + " will isolate due to positive test    ("
                        + str(numIsolated_positiveGroupmate)
                        + " as groupmates of positive)"
                    )
                    print(
                        "\t"
                        + str(numSelfIsolated_positiveContact)
                        + " will isolate due to positive contact ("
                        + str(numSelfIsolated_positiveContactGroupmate)
                        + " as groupmates of contact)"
                    )
