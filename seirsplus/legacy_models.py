...
class SEIRSNetworkModel:
    ...
    def run_iteration(self):
        ...
        self.N[self.tidx] = numpy.clip(
            (
                ...
                + self.numE[self.tidx]
                + self.numI[self.tidx]
                + self.numD_E[self.tidx]
                + self.numD_I[self.tidx]
                + self.numR[self.tidx]
            ),
            a_min=0,
            a_max=self.numNodes,
        ...
        if self.t >= self.tmax or (
            self.numI[self.tidx] < 1
            and self.numE[self.tidx] < 1
            and self.numD_E[self.tidx] < 1
            and self.numD_I[self.tidx] < 1
        ):
            self.finalize_data_series()
            return False
    ...
    def plot(
        ...
        Dseries = (
            (self.numD_E + self.numD_I) / self.numNodes
            if plot_percentages
            else (self.numD_E + self.numD_I)
        )
        ...
        if combine_D and (
            any(Dseries) and plot_D_E == "shaded" and plot_D_I == "shaded"
        ):
            ax.fill_between(
                numpy.ma.masked_where(Dseries <= 0, self.tseries),
                numpy.ma.masked_where(Dseries <= 0, Dseries),
